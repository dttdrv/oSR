...
"%CXX%" -IC:/Users/deyan/Projects/oSR/src -std=c++20 -O2 src/demo/dx12_wind_tunnel/main.cpp src/demo/dx12_wind_tunnel/display_upscale.cpp src/demo/dx12_wind_tunnel/dx12_texture_io.cpp src/demo/dx12_wind_tunnel/presenter.cpp src/demo/wind_tunnel/debug_dumps.cpp src/demo/wind_tunnel/synthetic_frame.cpp src/demo/wind_tunnel/synthetic_roi.cpp src/demo/wind_tunnel/temporal_diagnostics.cpp src/demo/wind_tunnel/temporal_resolve.cpp src/demo/wind_tunnel/sequence_metrics.cpp src/reconstruction/feature_locks.cpp src/reconstruction/trust_field.cpp src/core/frame_context.cpp src/core/quality_mode.cpp src/core/logging.cpp src/debug/capture_analysis.cpp src/debug/capture_pack.cpp src/debug/frame_context_readiness.cpp src/debug/validation.cpp src/backends/dx12/dx12_backend.cpp src/backends/dx12/debug_upscale_pass.cpp src/backends/dx12/temporal_resolve_pass.cpp src/backends/dx12/descriptor_helpers.cpp -ld3d12 -ldxgi -ldxguid -ld3dcompiler -luser32 -o "%OUT%"
if errorlevel 1 (
  echo.
  echo Build failed.
  pause
